...
function main(): void {
  ...
  runCommandCheck("Localnet suite passes", "bash", ["scripts/test-localnet.sh"]);
  runCommandCheck("SDK build passes", "npm", ["run", "build"], "sdk-package");
  runCommandCheck("SDK tests pass", "npm", ["test"], "sdk-package");
  runCommandCheck("SDK pack dry run passes", "npm", ["pack", "--dry-run"], "sdk-package");
  runCommandCheck("Relayer build passes", "npm", ["run", "build"], "relayer");
  runCommandCheck("Relayer tests pass", "npm", ["test"], "relayer");
  runCommandCheck("Agent kit build passes", "npm", ["run", "build"], "agent-kit-tool");
  runCommandCheck("Agent kit tests pass", "npm", ["test"], "agent-kit-tool");

  runCheck("Adversarial results matched expectations", verifyAdversarialResults);
  runCheck("Treasury config exists and is valid", verifyTreasuryConfig);
  runCheck("Ceremony hashes match frozen artifacts", verifyCeremonyHashes);
  runCheck("Rust verifying key matches verification_key_20.json", verifyRustVerifyingKey);
  runCheck("Pilot / pre-audit language is present in launch docs", verifyPilotLanguage);
  runCheck("Spend-key docs do not claim proofs are signed", verifySpendKeyLanguage);
  runCheck("Sensitive files are covered by .gitignore", verifyGitignoreCoverage);
...
function runCommandCheck(
  ...
  results.push({
    name,
    ok: false,
    detail: summarizeCommandFailure(command, args, result.stdout, result.stderr),
  });
}
...
function verifyAdversarialResults(): string {
  ...
  for (const file of files) {
    ...
    if (failed.length > 0) {
      throw new Error(
        `${file} contains unmet expectations: ${failed.map((entry) => entry.name ?? "unknown").join(", ")}`,
      );
    }
  }
...
function verifyTreasuryConfig(): string {
  ...
  const document = JSON.parse(fs.readFileSync(TREASURY_CONFIG_PATH, "utf8")) as {
  ...
  if (typeof document.generatedAt !== "string" || document.generatedAt.trim().length === 0) {
...
function verifyCeremonyHashes(): string {
  if (!fs.existsSync(CEREMONY_DOC_PATH)) {
    throw new Error("docs/CEREMONY.md is missing");
  }

  const document = fs.readFileSync(CEREMONY_DOC_PATH, "utf8");
  const requiredArtifacts = new Map<string, string>([
    ["withdraw_20_final.zkey", path.join(ROOT_DIR, "build", "withdraw_20_final.zkey")],
    ["verification_key_20.json", path.join(ROOT_DIR, "build", "verification_key_20.json")],
    ["withdraw_20.wasm", path.join(ROOT_DIR, "build", "withdraw_20_js", "withdraw_20.wasm")],
    ["withdraw_20.r1cs", path.join(ROOT_DIR, "build", "withdraw_20.r1cs")],
  ]);

  for (const [artifact, artifactPath] of requiredArtifacts) {
    const recordedHash = extractHashFromCeremonyDoc(document, artifact);
...
function verifyRustVerifyingKey(): string {
  ...
  if (generated.status !== 0) {
    throw new Error(summarizeCommandFailure("npx", ["tsx", "scripts/parse-vk.ts"], generated.stdout, generated.stderr));
  }
  ...
  if (generatedRust !== committedRust) {
    throw new Error("program verifying_key_20.rs does not match parse-vk output");
  }

  return "verifying_key_20.rs matches generated constants";
}

function verifyPilotLanguage(): string {
  const docPaths = [
  ...
  const missing = docPaths.filter((docPath) => {
    ...
    return !content.includes("pilot") && !content.includes("pre-audit");
  });

  if (missing.length > 0) {
    throw new Error(`missing pilot/pre-audit language in ${missing.map((file) => path.basename(file)).join(", ")}`);
  }
...
function verifySpendKeyLanguage(): string {
  ...
  if (offenders.length > 0) {
    throw new Error(`found forbidden wording in ${offenders.map((file) => path.basename(file)).join(", ")}`);
  }
...
function extractHashFromCeremonyDoc(document: string, artifact: string): string {
  const pattern = new RegExp(`\\|\\s*${escapeRegExp(artifact)}\\s*\\|\\s*([0-9a-f]{64})\\s*\\|`);
  const match = document.match(pattern);
  if (!match) {
    throw new Error(`Missing ${artifact} hash in docs/CEREMONY.md`);
  }
